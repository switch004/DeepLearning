...
class Accuracy(function.Function):
    ...
    def forward(self, inputs):
        ...
        if self.ignore_label is not None:
            ...
            pred = y.argmax(axis=1).reshape(t.shape)#predは予測値
            #index = np.where(chainer.cuda.to_cpu(t) != chainer.cuda.to_cpu(pred))
            index = xp.where(t != pred)
            correct = xp.where(t == pred)
            data = t[index]#正解ラベル
            data2 = pred[index]#予想ラベル
            corre = t[correct]
            return xp.asarray((pred == t).mean(dtype=y.dtype)), data, data2, index[0], pred, corre, correct[0]
